...
def find_urls(s):
    return re.findall('http[s]?://\S*\.\S{2,}', s)
    # regular expression finds urls based on the characters
...
def grab_headlines():
    ## For use with file: 'opinion.html'
    # f = open('opinion.html', 'r')
    # text = f.read()

    ## For use with url: 'http://www.michigandaily.com/section/opinion'
    url = 'http://www.michigandaily.com/section/opinion'
    result = requests.get(url)
    html_doc = result.content # the url requested html data is stored

    soup = BeautifulSoup(html_doc, 'html.parser') # soup is made
    #print(soup.prettify())

    tags = soup('ol') # tag for most read pulled out of soup
    for tag in tags: # tags made readable
        most_read = tag.text.strip().split('\n')
    return most_read
...
def get_umsi_data():
    umsi_titles = dict() # titles dictionary is initialized
    url = 'https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All'
    page = 0 # page number in directory

    while page < 13: # ability to iterate through all 12 pages of directory
        updated_url = url + "&page=" + str(page)
        result = requests.get(updated_url, headers={'User-Agent': 'SI_CLASS'})
        html_directory = result.content
        soup = BeautifulSoup(html_directory, 'html.parser')
        #print(soup.prettify())


        name_tags = soup.find_all('div', class_="field-item even", property="dc:title")
        title_tags = soup.find_all('div', class_="field field-name-field-person-titles field-type-text field-label-hidden")

        title_assign = 0 # iterates to next title to match the for loop iterating each name
        for name in name_tags: # titles dictionary is populated
            umsi_titles[name.text] = title_tags[title_assign].text
            title_assign += 1

        page += 1 # calls up the next title for next up name in the for loop
